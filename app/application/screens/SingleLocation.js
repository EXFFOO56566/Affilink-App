...
import { getDealsByLocation, getLocationById } from "../config/DataApp";
...
export default function SingleLocation(props) {
  ...
  useEffect(() => {
    ...
    getDealsByLocation(id).then(response => {
        setData(response);
    })
  ...
  const loadMore = () => {
    ...
    getDealsByLocation(id, page+1).then((response) => {
  ...
  if (isLoaded) {
   ...
   return (
    ...
    showsVerticalScrollIndicator={false}>
        ...
        <ImageBackground source={{uri: single.image}} style={Styles.single3Image}>
        <LinearGradient colors={['rgba(0,0,0,0.8)', 'rgba(0,0,0,0.3)']} style={Styles.single3Overlay}>
        <Text style={{fontSize: 24, color:'#fff'}}>{single.title}</Text>
